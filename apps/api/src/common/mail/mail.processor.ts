...
export class EmailProcessor extends WorkerHost {
  ...
  async process(job: Job): Promise<void> {
    ...
    switch (job.name) {
      case EmailJobName.VERIFICATION:
        await this.sendVerificationEmail(job.data as VerificationEmailData)
        break
      case EmailJobName.PASSWORD_RESET:
        await this.sendPasswordResetEmail(job.data as PasswordResetEmailData)
        break
      case EmailJobName.TEACHER_APPROVED:
        await this.sendTeacherApprovedEmail(job.data as TeacherApprovedEmailData)
        break
      default:
  ...
  private async sendVerificationEmail(data: VerificationEmailData) {
  ...
  private async sendPasswordResetEmail(data: PasswordResetEmailData) {
    const publicWebUrl = this.configService.get<string>('PUBLIC_WEB_URL') ?? ''
    const resetUrl = `${publicWebUrl}/reset-password?token=${data.token}&email=${data.email}`
  ...
  private async sendTeacherApprovedEmail(data: TeacherApprovedEmailData) {
    const publicWebUrl = this.configService.get<string>('PUBLIC_WEB_URL') ?? ''
    ...
    this.logger.log(`Teacher approved email sent to ${data.email}`)
  }
}
