...
export class MailService {
  ...
  async sendPasswordResetOtpEmail(email: string, name: string, otpCode: string) {
    try {
      await this.transporter.sendMail({
        from: `"SMS Platform" <${this.mailConfiguration.from}>`,
        to: email,
          ...
          <html>
            ...
            <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
              <div style="background-color: #f8f9fa; padding: 30px; border-radius: 10px;">
                ...
                <div style="text-align: center; margin: 30px 0; background-color: #e8f4f8; padding: 20px; border-radius: 5px;">
                  <p style="font-size: 14px; color: #666; margin-bottom: 10px;">Your password reset code:</p>
                  <p style="font-size: 36px; font-weight: bold; color: #dc3545; letter-spacing: 5px; margin: 0;">
                    ${otpCode}
                  </p>
                </div>
                <p style="font-size: 16px; margin-bottom: 20px; text-align: center;">
                ...
                <p style="font-size: 12px; color: #999;">
                  This password reset code will expire in 2 hours.
                </p>
              </div>
            </body>
          </html>
